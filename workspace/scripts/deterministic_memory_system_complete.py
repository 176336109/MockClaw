...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('/tmp/deterministic_memory_complete.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger('deterministic_memory_complete')

class DeterministicMemorySystemComplete:
    """确定性记忆系统完整实现"""
    ...
    def __init__(self):
        ...
        # 数据库
        self.db_path = self.memory_dir / "deterministic_memory_complete.db"
        self._init_database()
        
        logger.info("确定性记忆系统完整版初始化完成")
    ...
    def _init_database(self):
        ...
        # 索引
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_checkpoints_type_complete ON checkpoints(checkpoint_type)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_success_category_complete ON success_patterns(category)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_failure_category_complete ON failure_patterns(category)')
        cursor.execute('CREATE INDEX IF NOT EXISTS idx_consistency_status_complete ON consistency_checks(status)')
    ...
    def find_success_patterns(self, category: str = None, keyword: str = None, limit: int = 50) -> List[Dict]:
        """查找成功模式"""
        conn = sqlite3.connect(str(self.db_path))
        cursor = conn.cursor()
    ...
    def find_failure_patterns(self, category: str = None, keyword: str = None, limit: int = 50) -> List[Dict]:
        """查找失败模式"""
        conn = sqlite3.connect(str(self.db_path))
        cursor = conn.cursor()
